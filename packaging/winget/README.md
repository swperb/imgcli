...
manifest for `swperb.imgcli`, pinned to the **v0.5.0** Windows release
...
wingetcreate update swperb.imgcli `
  --version 0.5.0 `
  --urls https://github.com/swperb/imgcli/releases/download/v0.5.0/imgcli-windows-x86_64.zip `
  --submit
```

Or copy these files into `manifests/s/swperb/imgcli/0.5.0/` in a fork of
